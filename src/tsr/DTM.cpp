...
void denoise_points(std::vector<Point_3> &points) {
  ...
  std::cerr << "Keep " << k << " of " << indices.size() << " indices"
            << std::endl;
  {
...
void simplify_mesh(Surface_mesh const &source_mesh, Surface_mesh &target_mesh) {
  simplify_mesh(source_mesh, target_mesh, DEFAULT_COSINE_MAX_ANGLE_REGIONS,
...
void simplify_mesh(Delaunay_3 const &source_mesh, Delaunay_3 &target_mesh,
  ...
  simplify_mesh(*source_surface_mesh, *target_surface_mesh,
                cosine_max_angle_regions, max_distance_regions,
                cosine_max_angle_corners, max_distance_corners);
