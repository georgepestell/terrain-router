...
std::unique_ptr<std::vector<std::vector<Point_2>>> extract_feature_contours(cv::Mat &image, double simplification_factor, double uloffset_x, double uloffset_y, double cellsize_x, double cellsize_y) {
  ...
  cv::findContours(edges, cv_contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
    ...
    for (const auto& cv_contour : cv_contours) {
        ...
        for (const auto& point : new_contour) {
            // Convert OpenCV points to CGAL points
                        Point_2 cgal_point(round(point.x * cellsize_x + uloffset_x), round(uloffset_y - point.y * cellsize_y));
