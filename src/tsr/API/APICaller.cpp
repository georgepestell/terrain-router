...
/**
 * @brief Helper callback function for CURL which appends the request contents to a given output string
 * 
 ...
 */
size_t APICallbackHelper(void *contents, size_t size, size_t nmemb, std::string* output) {
...
void fetch_DEM_from_opentopography() {

}

APICaller::APICaller() {
    this->config = std::make_unique<APICallerConfig>();
}
...
std::string APICaller::fetchDataFromAPI(const std::string &url ) {
    ...
    if (!this->config->curl) {
        ...
        throw std::runtime_error("Cannot make API request: curl is not initialized");
    }
